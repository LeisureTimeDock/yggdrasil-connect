...
$filter->add('can_add_player', function (bool $can, string $name) {
    ...
    if ($algorithm === 'v3') {
        ...
        try {
            ...
            $uuid = $uuids[strtolower($name)] ?? UUID::generateUuidV3($name); // fallback
        } catch (\Throwable $e) {
            \Log::warning("UUID check fallback for '{$name}': " . $e->getMessage());
            $uuid = UUID::generateUuidV3($name); // fallback
        }
    } else {
        $uuid = Ramsey\Uuid\Uuid::uuid4()->getHex()->toString(); // 不常见情况
    }

    // 查询是否已存在该 uuid 或用户名
    if (UUID::where('uuid', $uuid)->orWhere('name', $name)->exists()) {
