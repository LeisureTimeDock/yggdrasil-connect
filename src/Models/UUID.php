...
class UUID extends Model
{
    ...
    public static function booted(): void
    {
        static::creating(function (UUID $model) {
            ...
            if ($algorithm === 'v3') {
                ...
                $model->uuid = static::fetchMojangUuid($model->name)
                    ?? static::generateUuidV3($model->name); // fallback
            } else {
                $model->uuid = RamseyUuid::uuid4()->getHex()->toString();
            }
        });
        ...
        static::updating(function (UUID $model) {
            ...
            if ($algorithm === 'v3') {
                ...
                $model->uuid = static::fetchMojangUuid($model->name)
                    ?? static::generateUuidV3($model->name);
            }
        });
    ...
    public static function fetchMojangUuid(string $name): ?string
    {
        try {
            $fetcher = new MojangUUIDFetcher();
            $result = $fetcher->fetchUUIDs([$name]);

            $lowerName = strtolower($name);
            return $result[$lowerName] ?? null;
        } catch (\Throwable $e) {
            \Log::warning("Mojang UUID fetch error for {$name}: " . $e->getMessage());
            return null;
        }
    }
}
